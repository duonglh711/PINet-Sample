...
class Agent(nn.Module):
    ...
    def load_weights(self):
        weight_path = os.path.join(self.p.save_path, "lane_detection_network.pkl")
        if os.path.exists(weight_path):
            self.lane_detection_network.load_state_dict(
                torch.load(self.p.save_path+'lane_detection_network.pkl', map_location='cuda:0'), False
            )
    ...
    def save_model(self, epoch, loss):
        ...
        torch.save(
            self.lane_detection_network.state_dict(),
            os.path.join(self.p.save_path, "lane_detection_network.pkl")
        )
