...
class Generator(object):
    ...
    def __init__(self, test_mode = False):
        ...
        with open("./train.txt") as f:
            while True:
                ...
                if os.path.isfile(line) and os.path.isfile(line[:-3]+'txt'):
                    with open(line[:-3]+'txt') as ff:   
                        jsonString = json.loads(ff.readline())
                        jsonString['raw_file'] = line
                        self.train_data.append(jsonString)


        self.size_train = len(self.train_data)

        print("train size: %d"%self.size_train)
        ...
        print("test size: %d"%self.size_test)
    ...
    def Resize_data_test(self, start, end):
        ...
        for i in range(start, end):
            ...
            temp_image = cv2.imread(data['raw_file'])
    ...
    def Resize_data(self, start, end, sampling_list):
        ...
        test_image = cv2.imread(self.test_data[test_index]['raw_file'])
