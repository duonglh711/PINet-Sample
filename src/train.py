...
def Training():
    ...
    for epoch in range(p.n_epoch):
        ...
        for inputs, target_lanes, target_h, test_image, data_list in loader.Generate(sampling_list):
            ...
            if step%200 == 0:
                lane_agent.save_model(int(step/200), loss_p)
        ...
        if epoch >= 0 and epoch%1 == 0:
            ...
            #     print("generate result")
                # test.evaluation(loader, lane_agent, index = idx, name="./test_result/test_result_"+str(epoch)+"_"+str(idx)+".json")

            # for idx in index:
            #     print("compute score")
            #     with open("./test_result/eval_result2_"+str(idx)+"_.txt", 'a') as make_file:
            ...
            #         make_file.write("\n")
            #     with open("./test_result/eval_result_"+str(idx)+"_.txt", 'a') as make_file:
            #         make_file.write( "epoch : " + str(epoch) + " loss : " + str(loss_p.cpu().data) )
            #         # make_file.write(evaluation.LaneEval.bench_one_submit("test_result_"+str(epoch)+"_"+str(idx)+".json", "test_label.json"))
            #         make_file.write("\n")
